...
import type { Node, ExecutionLog } from "@/types/estrategias"
...
export default function AutomacaoPage() {
  ...
  const addTimelineNode = useCallback(
    (type: string, day: number, position: { x: number; y: number }) => {
      const newNode: Node = {
        id: generateNodeId(),
        type,
        position,
  ...
  const updateNode = useCallback(
    (nodeId: string, updates: Partial<Node>) => {
      const updateNodes = (nodes: Node[]) => nodes.map((node) => (node.id === nodeId ? { ...node, ...updates } : node))
